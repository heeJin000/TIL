...
- [배열](https://github.com/heeJin000/TIL/blob/main/%EC%9E%90%EB%A3%8C%EA%B5%AC%EC%A1%B0/%EC%9E%90%EB%A3%8C%EA%B5%AC%EC%A1%B0.md#%EB%B0%B0%EC%97%B4)
  ...
  + [Doubly Linked List](https://github.com/heeJin000/TIL/blob/main/%EC%9E%90%EB%A3%8C%EA%B5%AC%EC%A1%B0/%EC%9E%90%EB%A3%8C%EA%B5%AC%EC%A1%B0.md#doubly-linked-list-%EA%B0%9C%EB%85%90)
<br><br><br><br>
...
```
- next를 반복하여 tail을 찾으면 자원과 시간이 들기 때문에 불리
- ArrayList은 배열을 가지며 마지막 값의 인덱스만 알면 되기 때문에 더 빠르게 삭제가 가능
<br><br><br>
...
numbers.addLast(20);
numbers.addLast(30);
System.out.println(numbers.size());     // 5가 삭제되어야 함
```

```
☕️ LinkedList.java
public int size(){
        return size;                // 여태껏 size를 관리했기 때문에 size만 호출하면 됨
}
```
<br><br><br>
...
numbers.addLast(20);
numbers.addLast(30);
System.out.println(numbers.indexOf(30));    // 30을 가진 인덱스는?
```
...
public int indexOf(Object data){
    Node temp = head;
    int index = 0;
    while(temp.data != data){       // 찾고자 하는 데이터가 맞지 않다면 탐색 반복
        temp = temp.next;           // 다음 Node로 넘어가면서 진행
        index++;

        if(temp == null){           // tail에 다다르면
            return -1;              // 검색 종료
        }
    }
    return index;
...
System.out.println(i.next());
System.out.println(i.next());
// LinkedList의 클래스의 내부 클래스인 ListIterator를 데이터 형으로 하는 객체를 리턴
```
```
☕️ LinkedList.java
public ListIterator listIterator(){
    return new ListIterator();  // 인스턴스화한 객체를 리턴
}

class ListIterator{
    private Node next;
    private Node lastReturned;  // 최종적으로 반환될 값을 담는 node
    private Node nextIndex;     

    ListItrator(){
        next = head;            // 역시 head에서 출발
    }
    public Object next(){
        lastReturned = next;    // 기존 값을 담아두고
        next = next.next;       // 이제 마음놓고 다음값을 담음
        nextIndex++;            
...
System.out.println(i.next());
System.out.println(i.hasNext());    // false (모든 노드가 출력 완료됐음)

while(i.hasNext()){                 // 다음 노드가 있나?
    System.out.println(i.next());   // 다음 노드 출력
}
```
...
☕️ LinkedList.java
public boolean hasNext(){
    return nextIndex < size();      // next()에서 nextIndex 값에 변화를 주기 때문에 
                                    // hasNext()는 추적이 가능하다
}
```

#### previous, hasPrevious?
- 결론적으로 LinkedList에서는 불가능하다.  
❗️ 노드는 바로 다음 노드만 알지 그 외는 알지 못한다. 이전 노드를 출력하는 행위는 할 수가 없는 것이다.  
    ➡️ 이중연결리스트로 해결 (다음 노드 뿐만 아니라 이전 노드까지 기억해야 하기 때문에 메모리를 더욱 차지)
<br><br>
...
☕️ Main.java
LinkedList.ListIterator i = numbers.listIterator();
i.add(5);               // next가 한번도 호출되지 않았으므로 노드가 특정되지 않은 상태임
                        // 5는 가장 첫 자리에 삽입됨
```
...
public void add(Object input){
    ...
    head = newNode;
    newNode.next = next;        // 원래 head 였던 노드가 다음에 위치

    lastReturned = newNode;
    nextIndex++;                // 다음에 새롭게 위치할 인덱스 값
    size++;                     // 전체 노드 갯수 증가
}
```
<br><br>
...
public void add(Object input){
    ...
    if(lastReturned == null){
     ...
     newNode.next = next; 
        ...
        lastReturned.next = newNode;        // lastReturned는 5이다.  
                                            // 5는 새로운 노드를 가리키게 됨
        newNode.next = next;                // 새로운 노드는 다음 노드인 10을 가리키게 됨
    }
...
☕️ LinkedList.java
public void remove(){
    if(nextIndex == 0){                      // 아무런 엘리먼트를 선택하지 않았다면
        throw new IllegalStateException();   // 예외를 던짐
    }
    LinkedList.this.remove(nextIndex-1);     // 이미 만들어져있는 remove를 사용
    nextIndex--;
}
```
- 삭제하려는 노드를 head부터 탐색하는 과정을 두 번 하게 됨
- 노드들이 자신의 이전 값을 알고 있다면 더 효율적으로 삭제가 가능
<br><br><br>
...
```
☕️ Main.java
ArrayList a = new ArrayList();      // 기존에 만들었던 것
a.addLast(10);
a.addLast(20);
a.addLast(30);
a.addLast(40);
...
LinkedList l = new LinkedList();    // 기존에 만들었던 것
l.addLast(10);
l.addLast(20);
l.addLast(30);
l.addFirst(5);                      // 1)
...
-  LinkedList  
새로운 노드가 head를 가리키게 한 뒤 새노드를 head로 지정
<br>
...
```
1) RAM의 특성으로 인해 모든 데이터에 항상 동일한 속도로 접근  
2) head부터 헤아려서 데이터를 찾음  
3) 모든 노드를 한 칸씩 뒤로 밀어야 함<br>
4) 앞 노드와 새 노드끼리 연결하면 끝
<br><br>
※ ArrayList 내부
```
public class ArrayList {
...
- ArrayList 내부에서 만든 배열의 크기가 100이라면  
데이터의 갯수는 그 크기를 초과할 수 없음 (➡️ 자바에서 자동으로 크기를 두 배 늘려주기 때문에 불편함은 없다)
- 반면에 LinkedList는 메모리 허용 범위에서 무한하게 데이터를 수용할 수 있음
<br><br><br><br>

# Doubly linked list 개념
<img width="560" alt="Linked" src="https://user-images.githubusercontent.com/86607510/168092506-67b4d694-bd4d-4ee2-836b-75f8304bd979.png">

▸ 장점  
- 다음 노드만 연결하던 linked list와 달리 이전 노드와도 연결이 가능
- 10개의 노드 중 3번째 노드는 next(head부터 진행), 8번째 노드는 previous(tail부터 진행)을 이용해 접근하므로  
훨씬 효과적  
...
### add, first, last
```
☕️ DoublyLinkedList.java
public void addFirst(Object input){    
        Node newNode = new Node(input);    
        newNode.next = head;

        if(head != null)        // head 있을 때만
        head.prev = newNode;    // 양방향 리스트이므로 이전 노드와의 연결도 신경써야 함             
...
public void addLast(Object input){
        ...
        if(size == 0){
            ...
            tail.next = newNode;
            newNode.prev = tail;    // 양방향이므로 이전 노드와 연결
            tail = newNode;                
            size++;
        }
...
public void add(int k, Object input){
        if(k == 0){
            ...
            Node newNode = new Node(input); 
            temp1.next = newNode;           // 새로운 노드와 연결. 기존 연결은 끊김    
            newNode.next = temp2;
            if(temp2 != null){              
                temp2.prev = newNode;       // temp2가 존재한다면 이전 노드와 연결
            }
            newNode.prev = temp1;           // 새 노드의 이전 노드로 temp1
            size++;                     
...
```
public Node node(int index){        
    if(index < size/2){         // 전체의 반(중간)보다 앞에 있으면
        Node x = head;          // head에서부터 찾는 게 빠름
        for(int i=0; i < index; i++ ){  
            x = x.next;
        }
    } else{                     // 그게 아니라면 tail에서부터 찾는 게 빠름
        Node x = tail;
        for(int i = size-1; i > index; i--){
            x = x.prev;
        }
...
public Object removeFirst(){
    ...
    temp = null;                // 노드의 next를 가리키는 연결을 끊음
    if(head != null){               
        head.prev = null;       // 노드의 prev를 가리키는 연결을 끊음
    }
    size--;
    return returnData;
}
...
public Object remove(int k){
    ...
    if(temp.next != null){
        temp.next.prev = temp;      // temp 다음 노드의 앞에 temp가 옴          
    }
    Object returnData = todoDeleted.data;
...
## Doubly Linked와 Iterator

### iterator()
- LinkedList는 다음 노드 탐색은 있었지만(next(), hasNext()) 이전 노드탐색은 없었음 array는 이전탐색 있었음 
    ➡️ 이전 노드 탐색을 위해서는 어딘가에 이전 노드 정보가 저장돼있어야 함
...
public boolean hasPrevious(){           // 이전 노드 호출 가능 여부
    return nextIndex < size();          
}

public Object previous(){
    if(next == null){                   // 가장 마지막 노드에 도달했다면
        lastReturned = next = tail;     // 가장 마지막 노드인 tail을 가르키게
    } else{
...
public void add(Object input){
    ...
    if(lastReturned == null){        // next()를 한번도 호출하지 않은 상태
     head = newNode;                
     newNode.next = next; 
    } else {                         // next()를 호출(lastReturned는 next를 가리키게 됨)
        lastReturned.next = newNode;   
        newNode.prev = lastReturned; // 이전 노드와 연결 생성
        if(next != null){            // next == null -> node 끝에 도달
            newNode.next = next;     // 끝에 도달 전까지 노드끼리 연결 관계 생성
            next.prev = newNode;     
        } else {
            tail = newNode;          // 끝에 도착하면 newNode는 tail이 되기만 하면 됨
        }
        // newNode.next = next;     /*필요없는 코드*/            
    }
...
```
### iterator()와 remove()
- lastReturned의 등장으로 이전 노드의 정보를 이미 따로 알고 있음
    ➡️ node()로 하나하나 헤아릴 필요x
```
public void remove(){
    if(nextIndex == 0){         // 한번도 next를 호출하지 않은 상태
    ...
    if(p == null){                  // 없애려는 노드가 맨 앞에 있다면 실행
        head = n;
        head.prev = null;           // 연결을 끊어버림
        lastReturned = null;        // 노드의 값 삭제
    } else {
        p.next = next;              // 삭제될 노드 다음 노드와 연결
        lastReturned.prev = null;   // 연결을 끊음
    }
    
    if(n == null){                  // lastReturned가 마지막 노드
        tail = p;
        tail.next = null;
    } else {
        n.prev = p;                 // 삭제될 노드와의 연결끊음
    }
    
    if(next == null){               // ❗️에서 NullPointException이 
        lastReturned = tail;        // 뜰 수 있기 때문에 if문으로 분기점 생성
    } else {
        lastReturned = next.prev;   // ❗️
    }
